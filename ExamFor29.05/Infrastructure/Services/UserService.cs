...
public class UserService(
    ...
    IRedisCacheService cacheService) : IUserService
{
    ...
    public async Task<PagedResponse<List<GetUserDto>>> GetAll(UserFilter filter)
    {
        try
        {
            ...
            var usersInCache = await cacheService.GetData<List<GetUserDto>>(cacheKey);
            if (usersInCache == null)
            {
                logger.LogInformation("Get users from database");
                var users = await context.Users.ToListAsync();
                usersInCache = users.Select(c => new GetUserDto
                {
                    Id = c.Id,
                    Email = c.Email,
                    Phone = c.PhoneNumber,
                ...
                await cacheService.SetData(cacheKey, usersInCache, 1);
            }

            if (filter.Name != null)
            {
                usersInCache = usersInCache.Where(u => u.UserName.Contains(filter.Name)).ToList();
            }

            var mapped = mapper.Map<List<GetUserDto>>(usersInCache);
