    ...
    IEmailService emailService) : IAuthService
{
    ...
    public async Task<Response<string>> ChangeUserRoleAsync(RoleChangerDto input)
    {
        try
        {
            logger.LogInformation("Start change role");
        ...
        var addResult = await userManager.AddToRoleAsync(user, input.Role);
        if (!addResult.Succeeded)
            return new Response<string>(HttpStatusCode.InternalServerError, "Failed to add new role");

        logger.LogInformation("End change role");
